...
async def active_lease(db_session: AsyncSession, org, unit, tenant):
    ...
    # Anchored to "next month" so none of the 6 generated schedules have
    # a due_date in the past — keeps status assertions stable over time.
    lease_start = date.today().replace(day=1) + relativedelta(months=1)
    lease_end = lease_start + relativedelta(months=5, day=31)
...
async def test_list_rent_schedules_flat(client: AsyncClient, active_lease):
    ...
    assert r.status_code == 200
    body = r.json()
    assert body["total"] >= 6  # 6 consecutive months from the fixture's anchor
    assert "totalPages" in body
