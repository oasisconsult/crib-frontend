...
function compactUGX(n: number) {
  if (n === 0) return "Free";
  if (n >= 1_000_000) return `UGX ${n / 1_000_000}M`;
  if (n >= 1_000)     return `UGX ${n / 1_000}k`;
    ...
    >
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12">
          <p className="text-sm font-semibold uppercase tracking-widest text-[#239487] mb-3">Pricing</p>
          <h2
            id="pricing-heading"
          ...
          {/* Billing toggle */}
          <div className="inline-flex items-center rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-1 gap-0.5">
            <button
              onClick={() => setAnnual(false)}
              className={cn(
                "px-4 py-2 text-sm font-medium rounded-md transition-all cursor-pointer",
                !annual
                  ? "bg-white shadow-sm text-[hsl(var(--foreground))] font-semibold"
        ...
        {/* Pricing cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start mt-4 pt-4">
          {PLANS.map(plan => {
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  plan.popular
                    ? "border-[#239487] shadow-lg ring-1 ring-[#239487]/20 scale-[1.02] z-10"
                    : "border-[hsl(var(--border))] shadow-sm hover:shadow-md hover:-translate-y-0.5",
                )}
              >
                {/* Popular badge */}
                {plan.popular && (
                ...
                <div className={cn("p-6 rounded-t-xl", plan.popular && "bg-[#f3fcfa]")}>
                  ...
                  <div className="h-5 mb-3">
                    {!isFree && annual && (
                      <p className="text-xs font-medium text-emerald-600">Save 20% with annual billing</p>
                    )}
                    {!isFree && !annual && (
                    ...
                    {plan.features.map(f => (
                      <li key={f} className="flex items-start gap-2.5 text-sm">
                        <div className="mt-0.5 h-4 w-4 rounded-full bg-[#f3fcfa] flex items-center justify-center shrink-0">
                          <Check className="h-2.5 w-2.5 text-[#239487]" aria-hidden />
                        </div>
        ...
        {/* Enterprise callout */}
        <div className="mt-10 rounded-xl border border-[hsl(var(--border))] bg-[#f9fafb] p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <p className="font-semibold text-[hsl(var(--foreground))]">Need Enterprise scale?</p>
            <p className="text-sm text-[hsl(var(--muted-foreground))]">
              Unlimited properties, dedicated support, API access, SSO — from UGX 1M/mo.
            </p>
          </div>
          <Button asChild variant="outline">
            <a href="#booking">Book a demo <ArrowRight className="h-4 w-4" /></a>
          </Button>
        </div>
