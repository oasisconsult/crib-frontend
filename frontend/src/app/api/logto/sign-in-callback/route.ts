...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("[sign-in-callback] Network error:", err);
    return NextResponse.redirect(
      new URL("/login?error=network_error", APP_URL),
    );
  }
  ...
  let idClaims: Record<string, unknown> = {};
  try {
    const idParts = (tokens.id_token as string | undefined)?.split(".");
    if (idParts?.[1]) {
      idClaims = JSON.parse(
  ...
  // ── Build response ───────────────────────
  const redirectTo =
    request.cookies.get("post_login_redirect")?.value ?? APP_URL;
  const response = NextResponse.redirect(new URL(redirectTo, APP_URL));
  ...
  // ✅ Set cookies with names middleware expects
  ...
  response.cookies.set("user_role", role, { ...cookieOpts, maxAge: expiresIn });
